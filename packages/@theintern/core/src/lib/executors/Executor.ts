 ...
 */
export interface Executor extends EventEmitter, Logger {
  ...
  getPlugin<Y extends keyof Plugins>(type: Y, name: string): Plugins[Y];
  ...
  loadScript(script: string | string[], isEsm?: boolean): Promise<void>;
  ...
  registerPlugin<T extends keyof Plugins>(
    type: T,
    name: string,
    init: PluginInitializer<Plugins[T]>
  ): void;
