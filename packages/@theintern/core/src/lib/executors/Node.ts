...
import {
  ...
  WebDriver,
  getDriverNames,
} from '@theintern/digdug';
import { Command, Server as LeadfootServer } from '@theintern/leadfoot';
import Remote from '../Remote';
import * as console from '../common/console';
import { pullFromArray } from '../common/util';
import { Config, EnvironmentSpec, isWebDriver } from '../config';
...
export default class Node extends Executor<NodeEvents, NodePlugins> {
  ...
  constructor(config?: ExecutorConfig) {
    ...
    process.on(
      'unhandledRejection',
      (reason: unknown | null | undefined, promise: Promise<any>) => {
  ...
  async resolveConfig() {
    ...
    config.suites = expandFiles(config.suites, config.basePath);
  ...
  protected _preLoad(): Promise<void> {
    ...
    if (!isTsLoaderIsEnabled()) {
      if (config.node.tsconfig) {
        const project = resolve(config.basePath, config.node.tsconfig);
        register({ project });
        this.log(
          'Registered ts-node with configured project',
          project,
          'in',
          config.basePath
        );
        ...
        if (
          ...
          register();
          this.log('Registered ts-node with implicit project');
        } else {
          ...
          if (existsSync(tsconfigPath)) {
            ...
            this.log('Registered ts-node with default project', tsconfigPath);
          }
        }
      }
    }
