...
import { Handle, createCancelToken } from '@theintern/common';
...
export default class RemoteSuite extends Suite {
  ...
  run(): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      let connectionOpened: () => void;
      let connectionErrored: (error: Error) => void;
      const pendingConnection = cancelToken.wrap(
        new Promise((resolve, reject) => {
          connectionOpened = resolve;
          connectionErrored = reject;
        })
      );
