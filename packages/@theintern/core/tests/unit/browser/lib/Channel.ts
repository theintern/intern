...
import _Channel, { ChannelOptions } from 'src/browser/lib/Channel';

let messages: string[];
let websocketError: 'construct' | 'send' | null;
let Channel: typeof _Channel;

registerSuite('browser/lib/Channel', {
  async before() {
    ({ default: Channel } = await mockImport(
      () => import('src/browser/lib/Channel'),
      (replace) => {
        replace(() => import('src/browser/lib/channels/WebSocket')).with(
          MockWebSocket as any
        );
        replace(() => import('src/browser/lib/channels/Http')).with(
          MockHttp as any
        );
      }
    ));
