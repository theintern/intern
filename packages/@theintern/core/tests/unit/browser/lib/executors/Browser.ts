...
import _Browser, { Config } from 'src/browser/lib/executors/Browser';
import { createConfigurator } from 'src/browser/lib';
...
registerSuite('browser/lib/executors/Browser', function () {
  ...
  const mockJson = sandbox.stub().resolves('{}');
  ...
  return {
    async before() {
      ({ default: Browser } = await mockImport(
        () => import('src/browser/lib/executors/Browser'),
        (replace) => {
          ...
          replace(() => import('src/browser/lib')).with(mockUtil);
