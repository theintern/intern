...
let WebSocket: typeof import('src/browser/lib/channels/WebSocket').default;

registerSuite('browser/lib/channels/WebSocket', function () {
  class MockWebSocket {
  ...
  return {
    async before() {
      ({ default: WebSocket } = await mockImport(
        () => import('src/browser/lib/channels/WebSocket'),
        (replace) => {
          replace(() => import('@theintern/common')).with({
            global: { WebSocket: MockWebSocket },
          });
          replace(() => import('src/lib/common/util')).with({
            parseUrl: () => ({} as any),
          });
        }
      ));
