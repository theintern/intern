...
import { BrowserName } from '@theintern/digdug';
    ...
    async before() {
      ({ default: Node } = await mockImport(
        ...
        (replace) => {
          ...
          replace(() => import('@theintern/leadfoot')).with({
            Command: MockCommand as any,
            Server: MockLeadfootServer as any,
          });
          replace(() => import('@theintern/digdug')).with({
            NullTunnel: MockTunnel as any,
            BrowserStackTunnel: MockTunnel as any,
            SeleniumTunnel: MockTunnel as any,
            getDriverNames(environments) {
              return environments.map((env) => env.browserName as BrowserName);
            },
            TestingBotTunnel: MockTunnel as any,
            CrossBrowserTestingTunnel: MockTunnel as any,
            SauceLabsTunnel: MockTunnel as any,
          });
        ...
        return {
          ...
          tests: {
            ...
            'good node_module'() {
              ...
              const module = require.resolve('execa');
              assert.isUndefined(
                require.cache[module],
                'expected test module not to be loaded already'
              );
              executor.loadScript('execa');
              assert.isDefined(
                require.cache[module],
                'expected module to have been loaded'
              );
