...
registerSuite('lib/node/util', function () {
  ...
  const mockFs = {
    readFile: ((
      filename: string,
      _encoding: any,
      callback: (error: Error | undefined, data?: string) => unknown
    ) => {
      if (fsData[filename]) {
    ...
    async before() {
      util = await mockImport(
        ...
        (replace) => {
          ...
          // replace(() => import('src/common/lib/global')).withDefault(
          //   mockGlobal
          // );
        }
      );
    ...
    tests: {
      expandFiles: {
        ...
        single() {
          ...
          assert.deepEqual(calls.sync[0], ['foo', { ignore: [], cwd: '/' }]);
        ...
        multiple() {
          ...
          assert.deepEqual(calls.sync, [
            ['foo', { ignore: [], cwd: process.cwd() }],
            ['bar', { ignore: [], cwd: process.cwd() }],
          ]);
          assert.property(calls.sync[0][1], 'cwd');
        ...
        negation() {
          ...
          assert.deepEqual(calls.sync, [
            ['foo', { ignore: ['bar', 'blah'], cwd: process.cwd() }],
            ['baz', { ignore: ['bar', 'blah'], cwd: process.cwd() }],
          ]);
