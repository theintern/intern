...
import Browser from 'src/browser/lib/executors/Browser';
...
export class EventHandler {
  handlers: { [event: string]: ((event?: unknown) => void)[] };
...
export class MockResponse extends EventHandler {
  ...
  end(data: string | undefined, callback?: (error?: Error) => unknown) {
    if (data) {
      this.data += data;
    }
    if (callback) {
