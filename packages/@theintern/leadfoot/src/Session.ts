...
> {
  ...
  constructor(
    ...
    this._logger = options.logger ?? { log: () => Promise.resolve() };
  }
    ...
    return this.serverPost<void>('frame', { id: id }).catch((error) => {
      if (
        ...
        (error.name === 'InvalidArgument' ||
          ...
          // with a message about an invalid tag name rather than an
          // InvalidArgument error (see
          // https://github.com/mozilla/geckodriver/issues/1456)
    ...
    if (this.capabilities.brokenParentFrameSwitch) {
      ...
      return this.serverPost<void>('frame/parent').catch((error) => {
        if (this.capabilities.brokenParentFrameSwitch == null) {
          this._logger.log('Error calling /frame/parent:', error);
