...
const baseAssert: QUnitBaseAssert = {
	...
	verifyAssertions: function(this: QUnitBaseAssert) {
		...
		if (
			!isNaN(this._expectedAssertions) &&
			this._numAssertions !== this._expectedAssertions
		) {
			throw new AssertionError(
				'Expected ' +
					this._expectedAssertions +
					' assertions, but ' +
					this._numAssertions +
					' were run'
			);
...
export const QUnit = {
	...
	config: <QUnitConfig>{
		...
		set autostart(value) {
			...
			if (!value) {
				autostartHandle = after(
					main.executor.config,
					'before',
					function(waitGuard?: Promise<void>) {
	...
	extend: function(
		...
		mixin: { [key: string]: any },
		skipExistingTargetProperties: boolean = false
	): { [key: string]: any } {
	...
	asyncTest: function(name: string, test: QUnitTestFunction): void {
		registerTest(name, function(this: any) {
			...
			const testAssert = delegate(baseAssert, {
				_expectedAssertions: NaN,
				_numAssertions: 0
			});
	...
	begin: function(callback: Function) {
		main.executor.reporterManager.on('runStart', function(
			executor: Executor
		) {
		...
		main.executor.reporterManager.on('runEnd', function(
			executor: Executor
		) {
			const numFailedTests = executor.suites.reduce(function(
				numTests: number,
				suite: Suite
			): number {
				return numTests + suite.numFailedTests;
			}, 0);
			const numTests = executor.suites.reduce(function(
				numTests: number,
				suite: Suite
			): number {
				return numTests + suite.numTests;
			}, 0);
			const numSkippedTests = executor.suites.reduce(function(
				numTests: number,
				suite: Suite
			): number {
				return numTests + suite.numSkippedTests;
			}, 0);
			const timeElapsed = Math.max.apply(
				null,
				executor.suites.map(function(suite: Suite): number {
