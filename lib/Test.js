define([
	...
	Test.prototype = {
		...
		get reporterManager() {
			return this.parent.reporterManager;
		},
		...
		run: function () {
			function handleTestError(error) {
				...
				if (error === SKIP) {
					self.reporterManager.emit('testSkip', self);
					finishRun();
					dfd.resolve();
				}
				else {
					self.error = error;
					self.reporterManager.emit('testFail', self);
					finishRun();
					dfd.reject(error);
				}
			}
			...
			function finishRun() {
				clearTimeout(timer);
				self.reporterManager.emit('testEnd', self);
			}
			...
			try {
				...
				self.reporterManager.emit('testStart', self);
				...
				Promise.resolve(result).then(function () {
					self.timeElapsed = Date.now() - startTime;
					self.hasPassed = true;
					self.reporterManager.emit('testPass', self);
					finishRun();
					dfd.resolve();
				}, handleTestError);
			}
