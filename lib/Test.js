define([
	...
	Test.prototype = {
		...
		/**
		 * Skips the test. By default, throws a TestSkipped error.
		 *
		 * @param {String} message
		 * If provided, will be stored on the test as a `skipped` property.
		 *
		 * @param {Boolean} doNotThrow
		 * If true, a TestSkipped error will not be thrown.
		 */
		skip: function (message, doNotThrow) {
			this.skipped = message || '';
			topic.publish('/test/skip', this);
			if (!doNotThrow) {
				var error = new Error();
				error.name = 'TestSkipped';
				throw error;
			}
		},
