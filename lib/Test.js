define([
	...
	Test.prototype = {
		...
		/**
		 * The remote environment that a test is running in.
		 * @see Suite#environmentType
		 */
		get environmentType() {
			return this.parent.environmentType;
		},
		...
		run: function () {
			function handleTestError(error) {
				...
				if (error === SKIP) {
					self.reporterManager && self.reporterManager.emit('testSkip', self);
					finishRun();
					dfd.resolve();
				}
				else {
					self.error = error;
					self.reporterManager && self.reporterManager.emit('testFail', self);
					finishRun();
					dfd.reject(error);
				}
			}
			...
			function finishRun() {
				clearTimeout(timer);
				self.reporterManager && self.reporterManager.emit('testEnd', self);
			}
			...
			try {
				...
				self.reporterManager && self.reporterManager.emit('testStart', self);
				...
				if (result && result.then) {
					...
					timer = setTimeout(function () {
						...
						result.cancel && result.cancel(reason);
					}, self.timeout);
				}
				...
				Promise.resolve(result).then(function () {
					self.timeElapsed = Date.now() - startTime;
					self.hasPassed = true;
					self.reporterManager && self.reporterManager.emit('testPass', self);
					finishRun();
					dfd.resolve();
				}, handleTestError);
			}
