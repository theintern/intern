	...
	Suite.prototype = {
		...
		get numTests() {
			function reduce(numTests, test) {
				return test.suite ? test.suite.length :
					test.tests ? test.tests.reduce(reduce, numTests) : numTests + 1;
			}
		...
		get numFailedTests() {
			function reduce(numFailedTests, test) {
				return test.suite ? test.suite.length - test.suite.filter('successful').length :
					test.tests ? test.tests.reduce(reduce, numFailedTests) :
