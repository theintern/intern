define([
	...
	Suite.prototype = {
		...
		/**
		 * The remote environment type that a suite is running in.
		 */
		get environmentType() {
			return (this.parent && this.parent.environmentType) ? this.parent.environmentType : this._environmentType;
		},

		set environmentType(value) {
			if (this._environmentType) {
				throw new Error('remote may only be set once per suite');
			}

			Object.defineProperty(this, '_environmentType', { value: value });
		},
		...
		run: function () {
			...
			function runNextTest() {
				...
				function runTest(test) {
					...
					// if `test` is actually a nested suite, beforeEach/afterEach should not be invoked (since they
					// are only supposed to be called for tests); passing null as the call name is the least
					// complicated way to do a no-op in this scenario
					call(test.tests ? null : 'beforeEach', test)
						.then(lang.bind(test, 'run'), handleTestError)
						.finally(function () {
							// `runNextTest` always needs to be executed when `test` is a suite, but if we try to
							// `return call()` and chain on the parent promise chain, `runNextTest` won’t be called when
							// `test` is a suite that fails due to an error in `afterEach`. This is because `call` will
							// return `undefined` when `test` is a suite resulting in the error getting passed through
							// `finally` to `handleTestError` instead.
							// So, since the parent promise chain is not used any more, we just attach the callbacks
							// directly here.
							call(test.tests ? null : 'afterEach', test).then(runNextTest, handleTestError);
						});
				}
			...
			function finishRun(error) {
				if (started) {
					...
					call('teardown').catch(function (teardownError) {
						if (!error) {
