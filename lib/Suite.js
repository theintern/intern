define([
	...
	Suite.prototype = {
		 ...
		 */
		get reporterManager() {
			return this.parent ? this.parent.reporterManager : this._reporterManager;
		},
		 ...
		 */
		get sessionId() {
			return this._sessionId ? this._sessionId :
				this.remote ? this.remote.sessionId :
				this.parent ? this.parent.sessionId :
				null;
		},
		...
		run: function () {
			...
			function handleSuiteLifecycleError(error, fromFinishRun) {
				self.error = error;
				self.reporterManager.emit('suiteError', self, error);
			...
			function finishRun(error) {
				if (started) {
					if (self.publishAfterSetup) {
						...
						self.reporterManager.emit('suiteEnd', self);
					}
			...
			if (!self.publishAfterSetup) {
				self.reporterManager.emit('suiteStart', self);
				started = true;
				startTime = Date.now();
			}
