define([
	...
	Suite.prototype = {
		...
		run: function () {
				...
				return new Promise(function (resolve, reject, progress, setCanceler) {
					...
					function next() {
						...
						function runWithCatch() {
							try {
								return Promise.resolve(suite[name] && suite[name](test));
							}
							catch (error) {
			...
			function setup() {
				return Promise.resolve(self.setup && self.setup());
			}
			...
			function teardown() {
				return Promise.resolve(self.teardown && self.teardown());
			}
			...
			return new Promise(function (resolve) {
				if (!self.publishAfterSetup) {
					resolve(start().then(setup));
				}
				else {
					resolve(setup().then(start));
				}
			})
			.then(runTests)
			.finally(function () {
