...
define([
	...
	'dojo/topic',
	'./util'
], function (wd, WebDriver, Element, wdUtils, when, Deferred, topic, util) {
	if (!wd) {
	...
	Object.keys(WebDriver.prototype).concat([ 'equals' ]).forEach(function (key) {
		...
		if (/* not a private interface */ key.charAt(0) !== '_') {
			PromisedWebDriver.prototype[key] = function () {
				var self = this,
					args = Array.prototype.slice.call(arguments, 0),
					// Use a local pointer to wrappedFunction so that subsequent calls to this function don't override
