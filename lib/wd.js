...
define([
	...
	'dojo/node!chai',
	'dojo/node!chai-as-promised',
	'dojo/promise/when',
	'dojo/Deferred',
	'dojo/topic',
	'./util'
], function (wd, WebDriver, Element, wdUtils, chai, chaiAsPromised, when, Deferred, topic, util) {
	if (!wd) {
	...
	// Figure out how to do this with the intern wrapper promises instead.
	 ...
	 *
	 * @property {string} proxyUrl The URL to the proxy server used by this remote session.
	 * @property {string} proxyBasePathLength The length of the filesystem path that should be truncated if a
	 * filesystem path is passed as a URL.
	 *
	...
	function PromisedWebDriver(config, desiredEnvironment) {
		this._wd = wd.promiseChainRemote(config);
