...
define([
	...
	'dojo-ts/topic',
	'./util'
], function (wd, WebDriver, Element, wdUtils, pathUtils, when, Deferred, JSON, topic, util) {
	if (!wd) {
		...
		// Always retrieve code coverage data before navigating to a new URL
			...
			wrappedFunction = (function (wrappedFunction) {
				return function () {
					...
					this._wd.execute('return typeof __teststackCoverage !== "undefined" && require("dojo-ts/json").stringify(__teststackCoverage)', function (error, returnValue) {
						if (error) {
							dfd.reject(error);
							return;
						}
