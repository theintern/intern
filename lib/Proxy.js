	...
	Proxy.prototype = {
		...
		_handler: function (request, response) {
			if (request.method === 'GET') {
				...
				request.on('end', function () {
					self._publishInSequence(JSON.parse(data)).then(function () {
		...
		_handleFile: function (request, response, instrument) {
			...
			if (
				...
				(
					...
					this.config.excludeInstrumentation.test(util.normalizePathForInstrumentation(file))
				)
			) {
				instrument = false;
			}
		...
		_publishInSequence: function (message) {
			...
			message.resolver = new Promise.Deferred(function (reason) {
				delete session.queue[message.sequence];
				throw reason;
			});
