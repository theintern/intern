define([
  ...
  'dojo/has!host-node?dojo/node!fs:dojo/request',
	'intern/browser_modules/cucumber/release/cucumber' // TODO: find out why just 'cucumber' does not work
	...
	function registerCucumber(featureSource, stepDefinitions) {
		main.executor.register(function(parentSuite) {
			var suite = new Suite({
				parent: parentSuite,
				run: function() {
					var runner = cucumber(featureSource, stepDefinitions);
	...
	registerCucumber.load = function load(feature, parentRequire, callback) {
		...
		if (requestor.readFile) {
			requestor.readFile(feature, function (err, featureSource) {
				...
				callback(function (stepDefinitions) {
					registerCucumber(featureSource, stepDefinitions);
				});
			});
		} else {
			requestor.get(require.toUrl(feature), { handleAs: 'text' }).then(function (featureSource) {
				callback(function (stepDefinitions) {
					registerCucumber(featureSource.data, stepDefinitions);
				});
			});
		}
	};
