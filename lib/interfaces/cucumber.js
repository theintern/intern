define([
	...
	function createSuite(featureSource, stepDefinitionInitializers) {
		function wrapInitializers() {
		...
		var suite = new Suite({
			run: function() {
				var runner = cucumber(featureSource, wrapInitializers);
	...
	function registerCucumber(featureSource) {
		// get all remaining arguments, skipping the featureSource
		var stepDefinitionInitializers = Array.prototype.slice.call(arguments, 1);
		main.executor.register(function(parentSuite) {
			var suite = createSuite(featureSource, stepDefinitionInitializers);
