define([
	...
	'./EnvironmentType'
], function (has, lang, Deferred, when, sourceMap, EnvironmentType) {
	...
	return {
		...
		normalizeStackTrace: function (error) {
			function getOriginalPosition(map, line, column) {
				...
				if (column === null) {
					return { line: realLine, column: null };
				}
				...
				return { line: originalPos.originalLine, column: originalPos.originalColumn };
			}
			...
			function getSource(path) {
				...
				if (source in coverageSourceMap) {
					...
					// TODO: Chrome/Node.js column is at the failing term
					// TODO: IE column is the beginning of the expression/line
					// TODO: Safari column number is just after the failing term
					//   need to go back one element in the mapping
					// TODO: Firefox, PhantomJS have no column number
					//   for no col number, find the largest original line number for the generated
					// TODO: SourceMap column numbers are 0-based, while browsers report 1-based

					console.log('orig pos for ' + line + ':' + col + ':', originalPos);
		...
		instrument: function (instrumenter, filedata, filepath, relativePath) {
			...
			// first, cleanup the map by removing entries for generated column 0
			// for (var i = data.map._mappings.length - 1; i >= 0; i--) {
			// 	var entry = data.map._mappings[i];
			// 	if (!entry.generatedColumn) {
			// 		data.map._mappings.splice(i, 1);
			// 	}
			// }
			...
			var smc = new sourceMap.SourceMapConsumer(data.map.toString()),
				map = [];
			...
			// coverageSourceMap[relativePath] = new sourceMap.SourceMapConsumer(data.map.toString());
			coverageSourceMap[relativePath] = map;
			return data.code;
		}
	};
});
