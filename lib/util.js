define([
...
], function (has, lang, Deferred, when, EnvironmentType, sourceMap, path, fs) {
	var slice = Array.prototype.slice,
		instrumentationSourceMap = {},
		fileSourceMaps = {},
		fileSources = {};
	...
	return {
		...
		/**
		 * Parse a stack trace, apply any source map mappings, and normalize its format
		 */
		normalizeStackTrace: function (error) {
			...
			function getSource(filepath) {
				...
				if ((match = /^\w+:\/\/[^\/]+\/(.*)$/.exec(filepath))) {
					// resolve the URL path to a filesystem path
					filepath = path.resolve(match[1]);
				}
				source = path.relative('.', filepath);

				// first, check for an instrumentation source map
				if (filepath in instrumentationSourceMap) {
					map = instrumentationSourceMap[filepath];
				...
				map = self.getSourceMap(filepath);
				if (map) {
					originalPos = getOriginalPosition(map, line, col);
					line = originalPos.line;
					col = originalPos.column;
			...
			// handle Chrome, Opera, and IE traces
			...
			// handle Safari and Firefox traces
			...
			var self = this,
				lines = error.stack.replace(/\s+$/, '').split('\n'),
				stack;
		...
		logError: function (error) {
			/*jshint maxlen:140, maxcomplexity:13 */
			if (error.message || error.stack) {
				...
				if (stack) {
					if (has('host-node')) {
						try {
							stack = this.normalizeStackTrace(error);
						} catch (e) {
							console.debug('Error normalizing trace: ' + e);
						}
					}
					else {
		 ...
		 */
		instrument: function (instrumenter, filedata, filepath) {
			...
			// `map` properties, where `code` is the instrumented code and `map` is a source map
			
			if (data.map) {
				this.processSourceMap(data.map.toString(), filepath, instrumentationSourceMap);
		...
		/**
		 * Load a process the source map for a given file
		 */
		getSourceMap: function (filepath) {
			...
			try {
				...
				if ((match = /# sourceMappingURL=(.*)$/.exec(lastLine))) {
					...
					return this.processSourceMap(data.toString('utf-8'), mapFile, fileSourceMaps);
				}
			} catch (e) {
				console.debug('Could not load source map for ' + filepath + ': ' + e);
				return null;
			}
		},
		 ...
		 * file path
		 */
		processSourceMap: function (data, filepath, store) {
