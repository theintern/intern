...
define([
	...
	/**
	 * Given a source object provide a list of key names that are arrays
	 * 
	 * @param source
	 * @return {Array}
	 */
	function listVectors(source) {
		var vectors = [];
	...
	return {
		...
		/**
		 * Builds permutations of an object(s) by flattening properties holding vector (Array) values into a collection
		 * of objects representing all combinations of objects for all vectors in the object.
		 *
		 * @param base {Object} a base set of properties applied to each source
		 * @param sources {Array.<Object>} a list of sources to flatten
		 * @return {Array.<Object>} a flattened collection of sources
		 */
		createPermutations: function (base, sources) {
			function flattenVector(name, vector, permutations) {
				return permutations.reduce(function (permutations, permutation) {
			...
			return sources.reduce(function (permutations, source) {
				var expansion = [ lang.mixin({}, base, source) ];
				var vectors = listVectors(source);

				if (vectors.length) {
					vectors.forEach(function (property) {
						var vector = source[property];
						expansion = flattenVector(property, vector, expansion);
					});
				}
				return permutations.concat(expansion);
			}, []);
		},
