...
define([
	...
	/**
	 * Return true if the module ID is a glob expression. This is similar to glob.hasMagic, but considers some special
	 * cases for AMD identifiers, like 'dojo/has!host-node?dojo/node!fs'.
	 */
	function isGlobModuleId(moduleId) {
		// Ignore empty moduleIds, absolute URLs, and loader plugins
		if (!moduleId || isAbsoluteUrl(moduleId) || moduleId.indexOf('!') !== -1) {
			return false;
		}

		// Return true if a glob special character or pattern is present in the module ID
