...
], function (has, lang, Deferred, when, EnvironmentType, pathUtil, hook) {
	return {
		/**
		 ...
		 */
		setInstrumentationHooks: function (config, instrumenter, basePath) {
			...
			function hookTransformer(code, filename) {
				return instrumenter.instrumentSync(code, pathUtil.resolve(filename));
			}
