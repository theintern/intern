define([
...
], function (has, lang, Deferred, when, EnvironmentType, Instrumenter, sourceMap, path, fs) {
	 ...
	 */
	function normalizeStackTrace(error) {
		...
		function getSource(tracepath) {
			...
			if ((match = /^\w+:\/\/[^\/]+\/(.*)$/.exec(tracepath))) {
				...
				tracepath = path.resolve(match[1]);
			}
			source = path.relative('.', tracepath);
		...
		var stack = /^\s+at /.test(lines[0]) ? processChromeTrace(lines) : processSafariTrace(lines);
		return '\n' + stack.join('\n');
	}
	...
	return {
		...
		logError: function (error) {
			/*jshint maxlen:140, maxcomplexity:15 */
			...
			try {
				...
				if (stack) {
					...
					if (has('host-node')) {
						try {
							stack = normalizeStackTrace(error);
							normalized = true;
						} catch (e) {
							console.warn('Error normalizing trace: ' + e);
						}
					}
		...
		instrument: function (filedata, filepath) {
			...
			if (data.map) {
				processSourceMap(data.map.toString(), filepath, instrumentationSourceMap);
