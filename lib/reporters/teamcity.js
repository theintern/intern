...
define([ '../util' ], function (util) {
	var teamcity = {
		...
		_sendMessage: function (type, args) {
			...
			args.timestamp = new util.Date().toISOString().slice(0, -1);
		...
		'/suite/start': function (suite) {
			...
			var startDate = teamcity._suiteStarts[suite.id] = new util.Date();
		...
		'/suite/end': function (suite) {
			...
			teamcity._sendMessage('testSuiteFinished', {
				name: suite.id,
				duration: new util.Date() - teamcity._suiteStarts[suite.id]
			});
		}
	};
