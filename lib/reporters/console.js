define([ 'dojo-ts/topic' ], function (topic) {
	...
	topic.subscribe('/suite/end', function (suite) {
		var numTests = suite.getNumTests(),
			numFailedTests = suite.getNumFailedTests();
	...
	topic.subscribe('/test/pass', function (test) {
		console.log('PASS: ' + (hasGrouping ? test.name : test.getId()) + ' (' + test.timeElapsed + 'ms)');
	});

	topic.subscribe('/test/fail', function (test) {
		console.error('FAIL: ' + (hasGrouping ? test.name : test.getId()) + ' (' + test.timeElapsed + 'ms)');
