...
define([
	...
	function getId(test) {
		var id = test.get('id');
		var reporter = pretty.reporters[test.get('sessionId')];
	...
	var pretty = {
		...
		'/suite/start': function (suite) {
			if (suite.name === 'main' && !suite.parent) {
				var reporter = pretty.reporters[suite.get('sessionId')];
		...
		'/test/skip': function (test) {
			pretty._record(test.get('sessionId'), SKIP);
			pretty.log.push('~ ' + getId(test) + ': ' + test.skipped);
		},
		'/test/pass': function (test) {
			pretty._record(test.get('sessionId'), PASS);
			pretty.log.push('✓ ' + getId(test));
		},
		'/test/fail': function (test) {
			var message = '× ' + getId(test);
			pretty._record(test.get('sessionId'), FAIL);
