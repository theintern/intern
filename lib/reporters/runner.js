define([
	...
	topic.subscribe('/session/start', function (remote) {
		sessions[remote.getSessionId()] = { remote: remote };
		console.log('Initialised ' + remote.environmentType);
	});

	topic.subscribe('/test/fail', function (test) {
		console.error('Test ' + test.getId() + ' FAILED on ' + sessions[test.getSessionId()].getRemote().environmentType + ':\n' + test.error.stack || test.error);
	});
	...
	topic.subscribe('/suite/end', function (suite) {
		if (suite.name === 'main') {
			sessions[suite.getSessionId()].suite = suite;
		}
	});

	topic.subscribe('/session/end', function (remote) {
		var session = sessions[remote.getSessionId()],
			suite = session.suite;
		...
		// grand total, however.
		console.log('%s: %d/%d tests failed', remote.environmentType, suite.getNumFailedTests(), suite.getNumTests());
	});
	...
	topic.subscribe('/runner/end', function () {
		...
		for (var k in sessions) {
			...
			++numEnvironments;
			numTests += session.suite.getNumTests();
			numFailedTests += session.suite.getNumFailedTests();
		}
