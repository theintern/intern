define([
	...
	'../args',
	'../util'
], function (Collector, Reporter, DetailedReporter, leadfootTopic, topic, args, util) {
	...
	// publish deprecation warnings from leadfoot's compat module
	...
	return {
		...
		'/test/fail': function (test) {
			console.error('Test ' + test.get('id') + ' FAILED on ' +
				sessions[test.get('sessionId')].remote.environmentType + ':');
		...
		'/suite/end': function (suite) {
			if (suite.name === 'main') {
				if (!sessions[suite.get('sessionId')]) {
					args.proxyOnly || console.warn('BUG: /suite/end was received for session ' + suite.get('sessionId') +
