define([
	...
	var appendFragment = function () {
		injectCSS();
		document.body.innerHTML = '';
		document.body.className = '';
		document.body.appendChild(fragment);

		// only append the fragment once
	...
	var htmlReporter = {
		...
		'/error': function (error) {
			...
			var link = '';
			if (error.lineNumber) {
				link = ' at <a href="'+ error.url + '">' + error.url + ':' +
					error.lineNumber + (error.columnNumber ? ':' + error.columnNumber : '') + '</a>';
				error = error.error;
			}

			td.innerHTML = '<h2>Error!</h2>' +
				'<p>An unhandled error occurred during testing' + link + ':</p>' +
				'<div><pre>' + util.getErrorMessage(error) + '</pre></div>';

			reportNode.appendChild(tr);
			appendFragment();
		}
	};
