define([
	...
	'dojo/topic',
	'../util'
], function (charm, nodeUtil, Collector, Reporter, DetailedReporter, topic, util) {
	var LIGHT_RED = 0x09;
	var LIGHT_GREEN = 0x0a;
	var LIGHT_YELLOW = 0x0b;
	...
	Runner.prototype = {
		...
		suiteEnd: function (suite) {
			if (suite.remote && !suite.hasParent) {
				if (!this.sessions[suite.sessionId]) {
					if (!this.config.intern.proxyOnly) {
						this.charm
							.foreground(LIGHT_YELLOW)
				...
				this.charm
					.foreground(numFailedTests > 0 ? LIGHT_RED : LIGHT_GREEN)
					.write(suite.name + ': ' + suite.numFailedTests + '/' + suite.numTests + ' tests failed')
					.display('reset')
					.write('\n');
			}
		},
		...
		testSkip: function (test) {
			this.charm
				.foreground('white')
				.write('~ ' + test.id + '(' + (test.skipped || 'skipped') + ')')
				.display('reset')
				.write('\n');
		},
