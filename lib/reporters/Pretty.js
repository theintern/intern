...
define([
	...
	Pretty.prototype = {
		start: function () {
			this.header = this.intern.config;
		...
		testSkip: function (test) {
			...
			this.log.push('~ ' + this._getId(test) + ': ' + (test.skipped || 'skipped'));
		},
		...
		_getReporter: function (suite) {
			if (!this.reporters[suite.sessionId]) {
				this.reporters[suite.sessionId] = new Report(suite.environmentType);
			}
		...
		_getId: function (test) {
			...
			if (reporter) {
				id = String(reporter.environment) + ' - ' + id;
			}
