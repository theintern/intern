define([
	...
	function HtmlReporter(config) {
		...
		function toggleCollapse(node) {
			var indented;
			var inflate = updateCollapseStatus(node);
		...
		function getIndentLevel(node) {
			...
			// return indent level or zero
			var indentLevel = indent ? parseInt(indent.slice('indent'.length), 10) : 0;
		...
		function updateCollapseStatus(node, inflate) {
			...
			node.classList[inflate ? 'remove' : 'add']('collapsed');
