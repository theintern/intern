define([
	...
	function HtmlReporter(config) {
		...
		function toggleCollapse(node) {
			var indented; // keep track of current indent level
			var inflate = updateCollapseStatus(node); // are we already collapsed
			var initialIndent = getIndentLevel(node); // set initial indent level

			// node will always be a suite to begin with, so we start the loop with its first child
			node = node.nextSibling;

			// node won't exist after the last test in a suite
			while (node) {
				indented = getIndentLevel(node); // set current indent level

				// break the loop if we're not indented
				if (indented === initialIndent) {
					break;
				}

				// if it's a suite, update the collapsed status
				if (node.classList.contains('suite')) {
					updateCollapseStatus(node, false);
				}

				// toggle display state
				if (indented - initialIndent === 1 && inflate) {
		...
		function getIndentLevel(node) {
			var child = node.children[1]; // second child always has a class of indentN
			var indent = child.className.split(' ').filter(function(name) {
				return name.indexOf('indent') >= 0;
			})[0]; // get the indentN class
			var indentLevel = indent ? parseInt(indent.slice('indent'.length), 10) : 0; // return indent level or zero

			return indentLevel;
		}

		function updateCollapseStatus(node, inflate) {
			// if we don't get an initial inflate, set it to be collapsed or not
			if (typeof inflate === 'undefined') {
				inflate = node.classList.contains('collapsed');
			}
