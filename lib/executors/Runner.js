define([
	...
	Runner.prototype = lang.mixin(Object.create(_super), /** @lends module:intern/lib/executors/Runner# */ {
		...
		_buildServerOptions : function(config) {
			var result = {};
			if (config.internServerOptions !== undefined && config.internServerOptions.proxy !== undefined) {
		...
		_createSuites: function (config, tunnel, overrides) {
			...
			var serverOptions = this._buildServerOptions(config);
			var server = new Server(tunnel.clientUrl, serverOptions);
