	...
	Runner.prototype = lang.mixin(Object.create(_super), /** @lends module:intern/lib/executors/Runner# */ {
		...
		_beforeRun: function () {
			...
			function loadTestModules() {
				self.suites = self._createSuites(config, self.tunnel, self.global.getArguments());
			...
			if (config.proxyOnly) {
				return promise.then(function () {
					// TODO: Add cancellation handler to allow the proxy to be closed
			...
			return util.flattenEnvironments(config.capabilities, config.environments).map(function (environmentType) {
				var suite = new Suite({
					...
					teardown: function () {
						...
						function endSession() {
							return reporterManager.emit('sessionEnd', command).then(function () {
