	...
	Runner.prototype = lang.mixin(Object.create(_super), /** @lends module:intern/lib/executors/Runner# */ {
		...
		run: function () {
			...
			function createAndStartProxy() {
				return self._createProxy(config).then(function (_proxy) {
			...
			function createSuites() {
				return self._createSuites(config, tunnel, self.global.getArguments()).then(function (suites) {
					self.suites = suites;
				});
			}

			function loadAndStartTunnel() {
				return self._loadTunnel(config).then(function (_tunnel) {
			...
			promise = promise
				.then(loadAndStartTunnel)
				.then(runSuites)
				.finally(function (valueOrError) {
					// If stopping the proxy or tunnel fails, getting that error is OK…
					return Promise.all([
						stopProxy(),
						stopTunnel()
					]).then(function () {
						// …otherwise, persist any earlier error or return value
