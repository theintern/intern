	...
	PreExecutor.prototype = {
		...
		getConfig: function (args) {
			...
			var promise = util.getModule(moduleId).then(function (config) {
				...
				else if (config.basePath.charAt(config.basePath.length - 1) !== '/') {
					config.basePath += '/';
				}
		...
		run: function () {
			...
			function getConfig() {
				return self.getConfig(args).then(function (_config) {
					...
					if (typeof config.useLoaders === 'object') {
						earlyEvents.push([
							'deprecated',
							'The "useLoaders" configuration option',
							'"loaders"'
						]);
						config.loaders = config.useLoaders;
					}
				});
			}
