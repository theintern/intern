define([
	...
	'../util',
	'require'
], function (aspect, has, lang, Promise, parseArgs, util, require) {
	/**
	...
	PreExecutor.prototype = {
		...
		getConfig: function (args) {
			...
			var promise = util.getModule(moduleId).then(function (config) {
				...
				if (typeof config.suites === 'string') {
					config.suites = [ config.suites ];
				}

				if (typeof config.functionalSuites === 'string') {
					config.functionalSuites = [ config.functionalSuites ];
				}
		...
		run: function () {
			...
			var promise = this
				...
				.then(function () {
					// allow the reporters to do any cleanup
					return executor.reporterManager.emit('stop');
				})
				.catch(function (error) {
					// report uncaught errors if the executor's reporterManager wasn't fully initialized
					if (!executor.reportersReady) {
