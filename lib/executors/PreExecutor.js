				...
				if (has('host-node')) {
					...
					[ 'suites', 'functionalSuites' ].forEach(function (name) {
						config[name].forEach(function (pattern, index) {
							files = globUtil.sync(pattern);

							// Original value is replaced by glob findings only if something was found.
							// This allow mixing glob patterns with regular paths.
							if (files.length > 0) {
								splice = [index, 1];
								files.forEach(function (filename) {
									splice.push(filename);
								});
								Array.prototype.splice.apply(config[name], splice);
							}
						});
					});
				}
