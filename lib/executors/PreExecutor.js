define([
	...
	'../util',
	'require'
], function (aspect, has, lang, Promise, request, pathUtil, globUtil, main, parseArgs, util, require) {
	/**
	...
	PreExecutor.prototype = {
		...
		getConfig: function (args) {
			...
			var promise = util.getModule(
				...
				[ 'suites', 'functionalSuites' ].forEach(function (name) {
					var jump = 0;
					var globCache = [];
					// To replace glob pattern with finding, first all findings needs to be parsed
					// and stored along with position they should be later inserted on
					config[name].forEach(function (pattern, index) {
						if (pattern.indexOf("*") > -1) {
							if (has('host-browser')) {
								throw new Error('Glob patters can be used in Node.js, but not in browser.');
							}
							else if (has('host-node')) {
								var files = globUtil.sync(pattern);

								if (files.length > 0) {
									globCache.push([index + jump, files]);
									jump += files.length - 1;
								}
							}
						}
					...
					// Modify original config by replacing glob patterns with glob findings
					globCache.forEach(function (cache) {
						Array.prototype.splice.apply(config[name], [cache[0], 1].concat(cache[1]));
					});
				});
