define([
	...
	'dojo/lang',
	'dojo/Promise',
	'dojo-core/has!host-browser?dojo/request',
	'dojo-core/has!host-browser?dojo/io-query',
	...
	PreExecutor.prototype = {
		...
		getArguments: function () {
			var kwArgs;
			if (has.default('host-browser')) {
				kwArgs = parseArgs.fromQueryString(location.search);
			}
			else if (has.default('host-node')) {
		...
		getConfig: function (args) {
			...
			var promise = util.getModule(
				...
				else if (has.default('host-browser')) {
					(function () {
				...
				if (has.default('host-browser') && args.loaders && args.loaders['host-browser']) {
		...
		run: function () {
			...
			function resolveSuites(loader) {
				...
				// Only try to g
				else if (has.default('host-browser') && config.suites.some(util.isGlobModuleId)) {
		...
		swapLoader: function (basePath, loaders, loaderOptions) {
			...
			return new Promise(function (resolve, reject) {
				if (has.default('host-node') && loaders['host-node']) {
				...
				else if (has.default('host-browser') && loaders['host-browser']) {
					global.require = global.define = undefined;
