define([
	...
	function PreExecutor(kwArgs) {
		...
		if (executorId.indexOf('/') === -1) {
			...
			executorId = require.toAbsMid('./lib/executors/' + executorId);
		}
	...
	PreExecutor.prototype = {
		...
		getConfig: function (args) {
			...
			var promise = util.getModule(moduleId).then(function (config) {
				config = lang.deepCopy(config, args);
		...
		_handleError: function (error) {
			if (has('host-browser') && location.pathname.replace(/\/+[^\/]*$/, '/').slice(-10) === '/__intern/') {
				...
				var htmlError = String(error.stack || error).replace(/&/g, '&amp;').replace(/</g, '&lt;');
		...
		run: function () {
			...
			var promise = this
				...
				.then(function (Executor) {
					var executor = new Executor(config, self);
					return executor.run();
				}, earlyErrorHandler);
