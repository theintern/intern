define([
	...
	Executor.prototype = {
		 ...
		 * should override this method to actually run tests.
		 *
		 * @returns {Promise.<void>}
		 */
		run: function () {
			return this._beforeRun()
				.then(lang.bind(this, '_runTests'))
				.then(lang.bind(this, '_afterRun'));
		},
		...
		_runTests: function (runTests) {
			...
			function emitRunEnd() {
				return reporterManager.emit('runEnd', self);
			}

			function emitRunStart() {
				return reporterManager.emit('runStart', self);
			}

			var promise = Promise.resolve()
				.then(emitRunStart)
				.then(function () {
					return runTests().catch(emitFatalError);
				})
				.finally(emitRunEnd);
