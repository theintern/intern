...
define([
	...
	return function (port, instrumenter, root) {
		...
		function handleFile(request, response, instrumenter) {
			...
			if (/^\/__teststack\//.test(request.url)) {
				// TODO: Fix Dojo loader to not add so many slashes to URLs and then get rid of the /+
				wholePath = path.resolve(require.toUrl('teststack/../'), file.replace(/^__teststack\/+/, ''));
			}
			else {
