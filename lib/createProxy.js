...
define([
	...
	'dojo/aspect',
	'./util'
], function (require, http, path, fs, aspect, util) {
	...
	return function (config) {
		...
		function publishInSequence(message) {
			...
			do {
				...
				config.reporterManager.emit.apply(config.reporterManager, message.payload);
			}
		...
		aspect.after(server, 'close', function () {
			...
			config.reporterManager.emit('proxyEnd', config);
		});
		...
		config.reporterManager.emit('proxyStart', config);
		return server;
	};
});
