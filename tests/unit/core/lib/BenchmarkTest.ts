...
function getTestFunction(
  ...
  const fullTestFunc: FullBenchmarkTestFunction = <FullBenchmarkTestFunction>(
    testFunc
  );
...
registerSuite('core/lib/BenchmarkTest', {
  'BenchmarkTest#test'() {
    this.timeout = 5000;
      ...
      return function() {
        ...
        return test.run().then(function() {
          assert.isAbove(
            count,
            0,
            methodName + ' should have been called at least one time'
          );
        });
      };
    }
