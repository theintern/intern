...
registerSuite('core/lib/interfaces/object', function() {
  ...
  return {
    async before() {
      objInt = await mockImport(
        () => import('src/core/lib/interfaces/object'),
        replace => {
          ...
          replace(() => import('src/core/lib/Suite')).withDefault(Suite);
          replace(() => import('src/core/lib/Test'))
            .transparently()
            .withDefault(Test);
        }
      );
