...
registerSuite('core/lib/Server', function() {
    ...
    async before() {
      ({ default: Server } = await mockImport(
        ...
        replace => {
          ...
          replace(() => import('src/core/lib/middleware/post')).withDefault(
            post as any
          );
          replace(() =>
            import('src/core/lib/middleware/unhandled')
          ).withDefault(unhandled as any);
          replace(() =>
            import('src/core/lib/middleware/finalError')
          ).withDefault(finalError);
