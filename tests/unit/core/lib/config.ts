...
import { duplicate } from 'src/common';
import { FileConfig } from 'src/core/lib/config';

const { suite, test, before, beforeEach } = intern.getPlugin('interface.tdd');

import * as _config from 'src/core/lib/config';

suite('core/lib/config', () => {
  let config: typeof _config;
  const sandbox = createSandbox();

  before(async () => {
    config = await mockImport(
      () => import('src/core/lib/config'),
      replace => {
  ...
  suite('createConfig', () => {
    const expectedDefault = {
      ...
      description: '',
      environments: [{ browserName: 'node' as 'node' }],
