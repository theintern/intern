			...
			'#start': {
				init() {
					...
					return server.start().then(() => {
						...
						assert.deepEqual(
							mockBodyParser.urlencoded.firstCall.args[0],
							{
								extended: true
							}
						);
				...
				'message handling': {
					...
					tests: {
						...
						'message handler rejection'() {
							return server.start().then(() => {
								const listener = sinon
									.stub()
									.rejects(new Error('bad message'));
								server.subscribe('foo', listener);
