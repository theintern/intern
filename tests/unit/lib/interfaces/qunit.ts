...
import QUnit from '../../../../lib/interfaces/qunit';
...
registerSuite({
	...
	beforeEach() {
		main.executor = new Executor({}, null);
	...
	'.asyncTest'() {
		...
		return rootSuite.run().then(function () {
			let suite = <Suite> rootSuite.tests[0];
			assert.isDefined(suite.tests[0].error,
				'async test should throw an error on failed assertion');
			assert.isDefined((<Test> suite.tests[1]).error, 'async test should fail without QUnit.start');
			assert.strictEqual((<Test> suite.tests[1]).error.message,
				'Timeout reached on qunit suite 1 - qunit async test 2',
				'async test should fail without QUnit.start with a timeout message');
			assert.strictEqual((<Test> suite.tests[2]).hasPassed, true,
				'async test should work with QUnit.start');
			assert.strictEqual((<Test> suite.tests[3]).hasPassed, true,
	...
	'.module': {
		...
		'should add setup and teardown methods'() {
			...
			assert.isUndefined((<Suite> rootSuite.tests[1]).afterEach, 'afterEach should not exist if no QUnit setup method was added');
			assert.isUndefined((<Suite> rootSuite.tests[1]).beforeEach, 'beforeEach should not exist if no QUnit teardown method was added');
		},
		...
		'should have a working lifecycle methods'() {
			...
			expectedResults.forEach(function (method) {
				(<any> moduleParams)[method] = function () {
					results.push(method);
				};
			});
			...
			lifecycleMethods.forEach(function (method) {
				(<any> rootSuite.tests[0])[method]();
			});

			assert.deepEqual(results, expectedResults,
				'QUnit interface methods should get called when corresponding Suite methods all called');
		}
	},
	...
	'.test': {
		'should create and push test'() {
			...
			assert.strictEqual((<Suite> rootSuite.tests[0]).tests[0].name, 'qunit test 1',
				'Module should register a test named "qunit test 1"');
			assert.strictEqual((<Suite> rootSuite.tests[0]).tests[0].parent.name, 'qunit suite 1',
		...
		'should be added to latest module'() {
			...
			assert.isUndefined((<Suite> rootSuite.tests[0]).tests[0],
				'There should not be any tests registered in module named "qunit suite 1"');
			assert.isDefined((<Suite> rootSuite.tests[1]).tests[0],
				'There be a test registered in module named "qunit suite 1"');
			assert.strictEqual((<Suite> rootSuite.tests[1]).tests[0].name, 'qunit test 1',
				'Module 2 should register a test named "qunit test 1"');
			assert.strictEqual((<Suite> rootSuite.tests[1]).tests[0].parent.name, 'qunit suite 2',
				'Test should be registered under module named "qunit suite 2"');
		},

		'should call the test function'() {
			const results: Array<typeof QUnit.assert> = [];
			...
			assert.instanceOf((<Suite> rootSuite.tests[0]).tests[0], Test,
	...
	'.config': {
		'.autostart': {
			...
			'enabled'() {
				...
				return Promise.resolve<void>(main.executor.config.setup(main.executor)).then(function () {
			...
			'enabled, then disabled'() {
				...
				assert.ok(main.executor.config.setup,
				...
				assert.isUndefined(main.executor.config.setup(main.executor),
		...
		'.module'() {
			...
			return rootSuite.run().then(function () {
				assert.isTrue((<Test> (<Suite> rootSuite.tests[0]).tests[0]).hasPassed, 'Matching module should run and pass');
				assert.strictEqual((<Test> (<Suite> rootSuite.tests[1]).tests[0]).skipped, 'grep',
		...
		'.requireExpects'() {
			...
			return rootSuite.run().finally(function () {
				...
				assert.isTrue((<Test> (<Suite> rootSuite.tests[0]).tests[0]).hasPassed, 'Test with `expect` should pass');
				assert.isFalse((<Test> (<Suite> rootSuite.tests[0]).tests[1]).hasPassed, 'Test without `expect` should fail');
			});
		}
	},
	...
	'events': {
		'begin'() {
			...
			return main.executor.run().then(function () {
				assert.strictEqual(actual, expected, 'Test suite should have 3 tests registered');
			});
		},
