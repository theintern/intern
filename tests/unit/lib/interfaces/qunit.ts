...
registerSuite({
	...
	beforeEach() {
		...
		rootSuite = new Suite({
			name: null,
			reporterManager: main.executor.reporterManager
		});
	...
	'test sanity checking': function() {
		assert.strictEqual(
			main.executor.suites.length,
			1,
			'There should be exactly one root suite'
		);
		assert.instanceOf(
			main.executor.suites[0],
			Suite,
			'Root suite 1 should be a suite instance'
		);
		assert.strictEqual(
			main.executor.suites[0].name,
			null,
			'Root suite 1 should have no name'
		);
	},
	...
	'.asyncTest'() {
		...
		QUnit.asyncTest('qunit async test 1', function(
			assertParam: QUnitBaseAssert
		) {
		...
		QUnit.asyncTest('qunit async test 3', function(
			assertParam: QUnitBaseAssert
		) {
			setTimeout(function() {
				assertParam.ok(true);
		...
		QUnit.asyncTest('qunit async test 4', function(
			assertParam: QUnitBaseAssert
		) {
			QUnit.stop();
			setTimeout(function() {
	...
	'.module': {
		...
		'should add setup and teardown methods'() {
			...
			assert.typeOf(
				(<Suite>rootSuite.tests[0]).afterEach,
				'Function',
				'afterEach of the created suite should have type "Function"'
			);
			assert.typeOf(
				(<Suite>rootSuite.tests[0]).beforeEach,
				'Function',
			...
			assert.typeOf(
				(<Suite>rootSuite.tests[1]).afterEach,
				'null',
				'afterEach of the created suite should have type "null" if not present'
			);
			assert.typeOf(
				(<Suite>rootSuite.tests[1]).beforeEach,
				'null',
		...
		'should have a working lifecycle methods'() {
			...
			assert.deepEqual(
				results,
				expectedResults,
				'QUnit interface methods should get called when ' +
					'corrosponding Suite methods all called'
			);
		}
	},
		...
		'.expect'() {
			...
			return rootSuite.run().then(function() {
				assert.strictEqual(
					results[0],
					1,
					'Base assert should have "1" expected assertions'
				);
				assert.strictEqual(
					results[1],
					1,
		...
		'.push'() {
			...
			QUnit.test('qunit test 1', function(assertParam) {
				...
				assert.throws(
					...
					AssertionError,
					'push should throw an assertion error on fail'
				);
			});
			...
			return rootSuite.run().then(function() {
				assert.strictEqual(
					results[0],
					1,
					'Base assert should have "1" assertion'
				);
			});
		},
	...
	'.test': {
		...
		'should call the test function'() {
			...
			const test0 = <Test>(<Suite>rootSuite.tests[0]).tests[0];
			assert.instanceOf(test0, Test, 'test 1 should be a Test Instance');

			return rootSuite.run().then(function() {
				assert.strictEqual(
					QUnit.assert.isPrototypeOf(results[0]),
					true,
	...
	'.config': {
		'.autostart': {
			...
			'enabled, then disabled'() {
				...
				assert.ok(
					(<any>main.executor.config).before,
		...
		'.module'() {
			...
			return rootSuite.run().then(function() {
				...
				assert.isTrue(
					test.hasPassed,
					'Matching module should run and pass'
				);
				assert.strictEqual(
					skippedTest.skipped,
					'grep',
		...
		'.requireExpects'() {
			...
			// This test should fail even though it has no failures because it
			// is missing `expects`
			...
			return rootSuite
				...
				.then(function() {
					const passedTest = <Test>(<Suite>rootSuite.tests[0])
						.tests[0];
					const failedTest = <Test>(<Suite>rootSuite.tests[0])
						.tests[1];
					assert.isTrue(
						passedTest.hasPassed,
						'Test with `expect` should pass'
					);
					assert.isFalse(
						failedTest.hasPassed,
						'Test without `expect` should fail'
					);
				});
		}
	},
	...
	'.extend': {
		'should have a working expect'() {
			...
			assert.deepEqual(
				testObject,
				{ a: 1, b: { c: 1 } },
				'Extended Object should be equal to expected one'
			);
			...
			assert.deepEqual(
				testObject,
				{ a: 1 },
				'Extended object should delete undefined props'
			);
	...
	events: {
		begin() {
			...
			return main.executor.run().then(function() {
				...
				assert.isDefined(
					runtime,
					'runtime should be defined on "done"'
				);
			});
		},
