...
registerSuite({
	...
	suiteEnd: {
		'successful suite'() {
			...
			const suite = new Suite({
				name: 'suite',
				tests: [new Test(<any>{ hasPassed: true })]
			});

			reporter.suiteEnd(suite);
			assert.lengthOf(
				mockConsole.messages['info'],
				1,
				'console.info should be called when the reporter suiteEnd method is ' +
					'called and there are no errors'
			);
			assert.match(
				mockConsole.messages['info'][0],
				new RegExp(
					'^' + suite.numFailedTests + '/' + suite.numTests + ' '
				),
	...
	fatalError() {
		...
		if (result.indexOf('No stack or location') === -1) {
			...
			let expected = has('host-node')
				? 'tests/unit/lib/reporters/Console.ts:71'
				: 'tests/unit/lib/reporters/Console.js';
