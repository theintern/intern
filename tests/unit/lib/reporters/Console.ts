...
registerSuite({
	...
	suiteEnd: {
		'successful suite'() {
			...
			const reporter = new Console({ console: mockConsole });
			const suite = new Suite({ name: 'suite', tests: [ new Test({ test() {} }) ] });
	...
	fatalError() {
		...
		if (result.indexOf('No stack or location') === -1) {
			...
			// was created above
			let expected = 'tests/unit/lib/reporters/Console.js:72';
