...
registerSuite({
	...
	'BenchmarkTest#test'(this: Test) {
		...
		const test = createTest(<BenchmarkTestConfig>{
	...
	'BenchmarkTest#constructor topic'() {
		...
		let actualTest: Test;
		let expectedTest = createTest(<BenchmarkTestConfig>{
	...
	'BenchmarkTest#testPass topic'() {
		...
		const expectedTest = createTest(<BenchmarkTestConfig>{
			name: 'foo',
			test: getTestFunction(function () {
				executionCount++;
			}, true)
	...
	'BenchmarkTest#skip'() {
		const test1 = createTest(<BenchmarkTestConfig>{
			name: 'skip 1',
			test: BenchmarkTest.skip(function () {}, 'foo')
		});
		const test2 = createTest(<BenchmarkTestConfig>{
			name: 'skip 2',
