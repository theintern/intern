...
import request, { Response } from 'src/common/lib/request';
import { createCancelToken, isCancel } from 'src/common';

registerSuite('common/lib/request', {
  beforeEach() {
    moxios.install();
  },
  ...
  tests: {
    ...
    cancel() {
      ...
      const req = request('test.html', { cancelToken });

      setTimeout(() => cancelToken.cancel());

      return req
        .then(() => {
          throw new Error('Request should not have completed');
        })
        .catch(error => {
          assert.isTrue(isCancel(error), 'Request should have been cancelled');
        });
    },
