...
registerSuite(function() {
	...
	return {
		...
		'navigation (#goBack, #goForward, #refresh)'(this: Test) {
			...
			const expectedUrl = util.convertPathToUrl(
				this.remote,
				toUrl('./data/default.html?second')
			);
			const expectedBackUrl = util.convertPathToUrl(
				this.remote,
				toUrl('./data/default.html?first')
			);
			...
			return {
				...
				' -> error'(this: Test) {
					...
					return session
						.get(toUrl('./data/scripting.html'))
						.then(function() {
							return session.executeAsync(function(
								done: Function
							) {
								/*global interns:false */
								done(interns());
		...
		'frame switching (#switchToFrame, #switchToParentFrame)'(this: Test) {
			...
			return session
				...
				.then(function(text: string) {
					...
					if (
						session.capabilities.scriptedParentFrameCrashesBrowser
					) {
		...
		'#findDisplayed'(this: Test) {
			...
			return session
				.get(toUrl('./data/visibility.html'))
				.then(function() {
		...
		'#doubleClick'(this: Test) {
			...
			return session
				...
				.then(function(result: any) {
					...
					assert.isArray(
						result.mouseup.a,
						'mouseup should have occurred'
					);
					assert.isArray(
						result.click.a,
						'click should have occurred'
					);
					assert.lengthOf(
						result.dblclick.a,
						1,
		...
		'#touchScroll'(this: Test) {
			...
			return session
				.get(toUrl('./data/scrollable.html'))
