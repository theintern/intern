...
registerSuite('Session', () => {
	...
	return {
		...
		tests: {
					...
					.then(function(text: string) {
						...
						if (
							session.capabilities
								.scriptedParentFrameCrashesBrowser
						) {
			...
			'window switching (#switchToWindow, #closeCurrentWindow)'(
				this: Test
			) {
					...
					if (session.capabilities.dynamicViewport) {
						return session
							...
							.then((size: Size) => {
								...
								if (
									!session.capabilities.brokenWindowMaximize
								) {
									return session
										.maximizeWindow()
			...
			'#find': (function() {
				...
				return {
					before() {
						...
						return session.get(
							'tests/functional/data/elements.html'
						);
					},
			...
			'#findDisplayed'(this: Test) {
				...
				return session
					...
					.then(function() {
						return session
							.findDisplayed('id', 'does-not-exist')
							.then(
								function() {
									throw new Error(
