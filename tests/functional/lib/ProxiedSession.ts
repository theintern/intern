...
registerSuite('ProxiedSession', function () {
	...
	function createProxiedSessionFromRemote(remote: Remote) {
		...
		const session = new ProxiedSession(remote.session.sessionId + '-test', server, remote.session.capabilities);
	...
	function createCoverageTest(method: 'get' | 'quit'): TestFunction {
		return function (this: Test) {
			...
			return task.then(() => {
				...
				_session.executor.emit = <any>function (eventName: string, value: any) {
				...
				assert.strictEqual(coverage.sessionId, _session.sessionId,
	...
	return {
		...
		tests: {
			...
			'#setHeartbeatInterval'() {
				...
				return _session.setHeartbeatInterval(50).then(function () {
